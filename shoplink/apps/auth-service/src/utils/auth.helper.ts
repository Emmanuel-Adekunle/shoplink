...
export const validateRegistrationData = (
  ...
  export const checkOtpRestrictions = async (email:string,next:NextFunction) => {
    if(await redis.get(`otp_lock:${email}`)){
        return next(new ValidationError("Account is Locked due to multiple failed attempts! Please Try Again After 30 minutes"
            
        )
    );
    }
    if(await redis.get(`otp_spam_lock:${email}`)) {
        return next(new ValidationError("Too many OTP requests! Please wait 1 Minute before requesting again! "
        )
    );
    }
    if(await redis.get(`otp_cooldown:${email}`)){
        return next(
            new ValidationError("Please wait 30 Seconds before requesting a new OTP!")
        );
    }
  };
  ...
  export const trackOtpRequests = async(email:string,next:NextFunction)=>{
    ...
    if(otpRequests >= 2 ){
        await redis.set(`otp_spam_lock:${email}`, "locked", "EX", 60) 
        // This Locks for 1 Minute
        return next(new ValidationError("Too many OTP requests. Please wait 1 Minute before requesting again."
